...
$id_division = $encryption->decrypt($_GET['id']);
// ==============================
// CEK LOGIN
// ==============================
...
$stmt_divisi = $conn->prepare("SELECT * FROM job_divisions WHERE id_division = ? LIMIT 1");
$stmt_divisi->bind_param("i", $id_division);
$stmt_divisi->execute();
...
<head>
  ...
  <title>Data Divisi - HR Management</title>
...
<div class="clearfix"></div>
  ...
  <div class="content-wrapper">
    ...
    <!-- Page Heading -->
    <h4 class="h4 mb-2 text-gray-800">Data Divisi</h4>
    <p class="h5"><?= $_SESSION["user"]["company_name"] ?></p>
        ...
        <?php endif; ?>
    <div class="row mt-3">
        <form action="divisi-update" method="POST" class="w-100" enctype="multipart/form-data"> <!-- form pembungkus -->
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <input type="hidden" name="id_division" id="id_division" value="<?= $id_division; ?>">
                        <div class="form-group">
                            <label for="kode_divisi">Kode Divisi</label>
                            <input type="text" class="form-control" id="kode_divisi" name="kode_divisi" placeholder="Kode Divisi" value="<?= $row_divisi['kode_divisi']; ?>">
                        </div>
                        <div class="form-group">
                            <label for="nama_divisi">Nama Divisi</label>
                            <input type="text" class="form-control" id="nama_divisi" name="nama_divisi" placeholder="Nama Divisi" value="<?= $row_divisi['nama_divisi']; ?>">
                        </div>
                        
                        <div class="form-group">
                            <label for="deskripsi">Deskripsi</label>
                            <textarea class="form-control" id="deskripsi" name="deskripsi" rows="3"
                                placeholder="Deskripsi lengkap"><?= $row_divisi['deskripsi'] ?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="status_divisi">Status</label>
                            <select class="form-control" id="status_divisi" name="status_divisi">
                                <option value="">-- Pilih Status --</option>
                                <option value="active" <?= $row_divisi['status']=='active'?'selected':'' ?>>Aktif</option>
                                <option value="inactive" <?= $row_divisi['status']=='inactive'?'selected':'' ?>>Non Aktif</option>
                            </select>
                        </div>
                    </div>
                </div>
